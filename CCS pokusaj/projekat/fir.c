...
Int16 fir_basic(Int16 input, Int16* coeffs, Int16 *history, Uint16 n_coeff)
{
	...
	for (i = 0; i < n_coeff; i++)
	{
		ret_val = _smac(ret_val, coeffs[i] >> 3, history[i]);
	}

    return (Int16)(ret_val >> 16);
}

Int16 fir_circular(Int16 input, Int16 *coeffs, Int16 *history, Uint16 n_coeff, Uint16 *p_state, Uint16 shift)
{
	Int16 i;
    Uint16 state;
    Int32 ret_val;
	...
	{
		ret_val = _smac(ret_val, coeffs[i] >> shift, history[state]);
